...
def path_search(graph, start, goal, heuristic_func=None):
    ...
    if start == goal:
        return 0.0, [start], {"nodes_expanded": 0, "max_frontier_size": 0, "exec_time": 0}
    ...
    # Track best known cost for each node
    best_known = {start: 0.0}
    ...
    while frontier:
        ...
        for neighbor, edge_cost in graph["edges"].get(current, {}).items():
            ...
            if neighbor not in g_scores or tentative_g < g_scores[neighbor]:
                ...
                # Calculate priority
                if heuristic_func:
                    h_value = heuristic_func(neighbor)
                    priority = tentative_g + h_value
                else:
                    priority = tentative_g

                # Only add to frontier if it's the best known path
                if neighbor not in best_known or tentative_g < best_known[neighbor]:
                    best_known[neighbor] = tentative_g
